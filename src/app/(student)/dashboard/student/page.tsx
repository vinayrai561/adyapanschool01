...
/* â”€â”€â”€ Certificate Card â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
        ...
        <div className="flex-1 min-w-0">
          ...
          {isComplete && certificate ? (
            <p className="text-xs text-green-600 font-semibold mt-0.5">
              Certificate Ready Â· ID: {certificate.certificateId}
            </p>
          ) : isComplete ? (
            <p className="text-xs text-green-600 font-semibold mt-0.5">Certificate Ready</p>
          ) : (
            <p className="text-xs text-gray-500 mt-0.5">
              {progress.progressPercent}% complete â€” {progress.totalLessons - progress.completedLessons.length} lessons remaining
            </p>
          )}
        </div>
      </div>
...
function CourseCard({
  ...
  /* Find "continue" lesson â€” first incomplete */
                    ...
                    <div key={mod._id} className="rounded-xl border border-gray-100 overflow-hidden">
                      <div className="flex items-center justify-between px-4 py-2.5 bg-gray-50">
                        <span className="text-xs font-bold text-gray-700">
                          Module {mi + 1}: {mod.title.replace(/^Module \d+ â€” /, '')}
                        </span>
...
function StudentDashboardInner() {
  ...
  const fetchTemplates = async () => {
    ...
    try {
      ...
      /* silent â€” templates are non-critical */
    } finally {
  ...
  const handleTemplateDownload = async (type: string, title: string) => {
    ...
    try {
      const res = await fetch(`/api/certificate-templates/${type}/download`, { credentials: 'include' });
      const blob = await res.blob();
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-orange-50 to-amber-50">
        <div className="text-center">
          ...
          <p className="text-gray-500 text-sm font-medium">Loading your dashboardâ€¦</p>
        </div>
      </div>
    );
  }
                          ...
                          <div key={item.enrollment.id}>
                            ...
                            <div className="flex items-center justify-between mt-1">
                              <p className="text-xs text-gray-400">
                                {item.progress.completedLessons.length} of {item.progress.totalLessons} lessons Â· {item.course.duration}
                              </p>
                              {isComplete && (
                  ...
                  {completedCourses === 0 && (
                    <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-2xl p-4 mb-6">
                      ...
                      <p className="text-sm text-amber-700">
                        You haven't completed any courses yet. Keep learning â€” certificates unlock automatically when you finish all lessons.
                      </p>
                    </div>
                  )}
                            ...
                            <div className="p-4">
                              ...
                              <div className="flex gap-2">
                                ...
                                >
                                  ...
                                  {isDownloading ? 'Savingâ€¦' : 'Download'}
                                </button>
                              </div>
                            </div>
                          </motion.div>
                  ...
                  >
                    <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        >
                          <Download className="w-3.5 h-3.5" />
                          {downloading === `tpl-${previewTemplate.type}` ? 'Downloadingâ€¦' : 'Download'}
                        </button>
                        <button
                          onClick={() => setPreviewTemplate(null)}
