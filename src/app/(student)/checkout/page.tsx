...
/* ── coupons — validated on backend only ── */
...
function CheckoutPageInner() {
  ...
  /* ── apply coupon — backend validated ── */
  ...
  const applyCoupon = async () => {
    ...
    try {
      const res = await fetch('/api/coupons/validate', {
        ...
        body: JSON.stringify({ code, planId: planKey, amount: basePrice }),
      });
      const data = await res.json();
      if (!data.success) {
        setCouponError(data.error || 'Invalid coupon');
      } else {
        setCouponApplied({ code: data.code, label: data.description, discount: data.discountAmount });
        setCouponSuccess(`🎉 "${data.code}" applied — ${data.description}`);
      }
    } catch {
            ...
            <>
              <div className="flex gap-2">
                ...
                <button onClick={applyCoupon} disabled={couponLoading}
                  className="rounded-xl bg-orange-500 text-white px-4 py-2 text-xs font-bold hover:bg-orange-600 transition-colors disabled:opacity-60 disabled:cursor-not-allowed flex items-center gap-1">
                  {couponLoading ? <span className="w-3 h-3 border-2 border-white/40 border-t-white rounded-full animate-spin" /> : null}
                  APPLY
                </button>
              </div>
