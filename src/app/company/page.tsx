...
export default function CompanyPage() {
  const [activeTab, setActiveTab] = useState<'hire' | 'post'>('hire');
  ...
  const postBenefits = ['Post tasks or projects in minutes','Receive proposals from students','Review, collaborate & select','Get quality work, on time'];
  const features = [
  ...
  const steps = [
    { n: '01', title: 'Post Your Need', desc: 'Describe the role or task in minutes',
          ...
          <div className='grid grid-cols-1 lg:grid-cols-2 gap-16 items-center'>
            <motion.div initial={{ opacity:0, x:-60 }} animate={{ opacity:1, x:0 }} transition={{ duration:0.9, ease:[0.22,1,0.36,1] }}>
              ...
              <motion.h1 initial={{ opacity:0, y:30 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.2, duration:0.8 }}
                ...
                <span className='text-orange-500'>Hire</span> <span className='text-gray-900'>Talent or</span><br />
                <span className='text-orange-500'>Post</span> <span className='text-gray-900'>Work.</span>
              </motion.h1>
              <motion.p initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.3 }}
                className='text-gray-500 text-xl leading-relaxed mb-10 max-w-lg'>
                Hire skilled, job-ready students or post real-world tasks and get it done by our talented community.
              </motion.p>
              <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.4 }}
                className='grid grid-cols-1 sm:grid-cols-2 gap-5'>
                <MovingBorderCard className={activeTab==='hire' ? 'shadow-xl shadow-orange-200' : ''}>
                  <motion.div whileHover={{ scale:1.01 }} onClick={() => setActiveTab('hire')}
                    className={'p-5 rounded-2xl cursor-pointer h-full transition-colors ' + (activeTab==='hire' ? 'bg-orange-50' : 'bg-white hover:bg-orange-50/50')}>
                    <div className='flex items-center gap-3 mb-3'>
                      ...
                      <h3 className='font-bold text-gray-900 text-base'>Hire <span className='text-orange-500'>Talent</span></h3>
                    </div>
                    <p className='text-xs text-gray-500 mb-3 leading-relaxed'>Find and hire job-ready students trained in in-demand skills.</p>
                    <ul className='space-y-2 mb-4'>
                      {hireBenefits.map(b => (
                        <li key={b} className='flex items-center gap-2 text-xs text-gray-600'>
                          <CheckCircle className='w-3.5 h-3.5 text-orange-500 flex-shrink-0' />{b}
                        </li>
                      ))}
                    </ul>
                    <Link href='/company/hire-talent'
                      className='inline-flex items-center gap-1.5 text-xs font-bold text-orange-600 border-2 border-orange-400 rounded-xl px-4 py-2 hover:bg-orange-500 hover:text-white hover:border-orange-500 transition-all'>
                      Browse Talent <ArrowRight className='w-3.5 h-3.5' />
                    </Link>
                  </motion.div>
                </MovingBorderCard>
                <MovingBorderCard className={activeTab==='post' ? 'shadow-xl shadow-orange-200' : ''}>
                  <motion.div whileHover={{ scale:1.01 }} onClick={() => setActiveTab('post')}
                    className={'p-5 rounded-2xl cursor-pointer h-full transition-colors ' + (activeTab==='post' ? 'bg-orange-50' : 'bg-white hover:bg-orange-50/50')}>
                    <div className='flex items-center gap-3 mb-3'>
                      <div className='w-10 h-10 rounded-xl bg-orange-100 border-2 border-orange-300 flex items-center justify-center'>
                        <Send className='w-5 h-5 text-orange-500' />
                      </div>
                      <h3 className='font-bold text-gray-900 text-base'>Post <span className='text-orange-500'>Work</span></h3>
                    </div>
                    <p className='text-xs text-gray-500 mb-3 leading-relaxed'>Post tasks or projects and get it done by our talented students.</p>
                    <ul className='space-y-2 mb-4'>
                      {postBenefits.map(b => (
                        <li key={b} className='flex items-center gap-2 text-xs text-gray-600'>
                          <CheckCircle className='w-3.5 h-3.5 text-orange-500 flex-shrink-0' />{b}
                    ...
                    <Link href='/hiring'
                      className='inline-flex items-center gap-1.5 text-xs font-bold text-orange-600 border-2 border-orange-400 rounded-xl px-4 py-2 hover:bg-orange-500 hover:text-white hover:border-orange-500 transition-all'>
                      Post Work <ArrowRight className='w-3.5 h-3.5' />
                    </Link>
                  </motion.div>
                </MovingBorderCard>
              </motion.div>
      ...
      <section className='py-24 bg-white relative overflow-hidden'>
        ...
        <div className='max-w-4xl mx-auto px-6 text-center relative z-10'>
          <motion.div initial={{ opacity:0, y:30 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }}>
            ...
            <p className='text-gray-500 text-xl mb-10'>Post your first task and start building your dream team today</p>
            <div className='flex flex-col sm:flex-row gap-4 justify-center mb-10'>
              <motion.div whileHover={{ scale:1.05, boxShadow:'0 24px 48px rgba(249,115,22,0.35)' }} whileTap={{ scale:0.97 }}>
                <Link href='/company/hire-talent'
                  ...
                  <Search className='w-5 h-5' /> Browse Talent
                </Link>
              </motion.div>
              <motion.div whileHover={{ scale:1.05 }} whileTap={{ scale:0.97 }}>
                <Link href='/hiring'
                  className='inline-flex items-center gap-2.5 px-10 py-4 rounded-2xl font-bold text-orange-600 text-lg border-2 border-orange-400 bg-white hover:bg-orange-50 transition-colors'>
                  <Send className='w-5 h-5' /> Post a Task
                </Link>
              </motion.div>
            </div>
